...
class GUMP
{
    ...
    {
        if ($lang) {
            $lang_file = __DIR__.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.$lang.'config.php';
                    ...
                    foreach ($rules as $rule) {
                        ...
                        if (is_callable(array($this, $method))) {
                            ...
                            if (is_array($result) && count($result) > 0 ) {
                                ...
                                if (count(array_column($result, 'field')) === 0) {
    ...
    {
        $lang_file = __DIR__.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.$this->lang.'config.php';
    ...
    protected function validate_none_zero($field, $input, $param = null)
    {
        if (!isset($input[$field])) {
            return false;
        }

        settype($input[$field],'integer');
        if (0 !== $input[$field]) {
            return false;
        }
    ...
    protected function validate_date($field, $input, $param = null)
    {
        ...
        $cdate2 = date('Y/m/d', strtotime($input[$field]));
     ...
     *
     * Usage: '<index>' => 'time,/your-time-expression/'
     *
     * @param string $field
     * @param array  $input
     *
    ...
    protected function validate_time($field, $input, $param = null)
    {
        ...
        if (!preg_match($time, $input[$field])) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
